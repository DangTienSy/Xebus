...
namespace QuanLyBanVeXe
{
    public class Connection
    {
        ...
        string connnectionString = @"Server=MARU\SQLEXPRESS;Database=QLVeXe;User ID=sa;Password=123";
        ...
        public bool KTKhoaChinh(string sql)
        {
            //ThucThiSQL("SELECT *FROM" + sql);
           DataTable DT= ExecuteData("SELECT *FROM " + sql);
           if (DT.Rows.Count > 0)
               return true;
           else
               return false;
        ...
        public bool KTKhoaNgoai(string sql)
        {
            //ThucThiSQL("SELECT *FROM" + sql);
            DataTable DT = ExecuteData("SELECT *FROM " + sql);
            if (DT.Rows.Count > 0)
